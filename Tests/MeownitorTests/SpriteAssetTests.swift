...
final class SpriteAssetTests: XCTestCase {
  ...
  func testBundledCatsContainFifteenSevenFrameStrips() throws {
    ...
    let directories = [
      resources.appendingPathComponent("ElsaHD/runtime"),
      resources.appendingPathComponent("Cats/K01/strips"),
      resources.appendingPathComponent("Cats/K02/strips"),
      resources.appendingPathComponent("Cats/K03/strips"),
    ]
    guard directories.allSatisfy({ FileManager.default.fileExists(atPath: $0.path) }) else {
    ...
    for directory in directories {
      ...
      XCTAssertEqual(urls.count, 15, directory.path)
      for url in urls {
