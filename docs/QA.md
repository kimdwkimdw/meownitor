...
- Elsa·K01·K02·K03 WebP 각 15개, 각 `8778×1254`, 7프레임과 투명 모서리
- 팩 카탈로그 파싱, 설치 상태, 삭제
- 앱 번들에 기본 4종 WebP 60개만 있고 다른 고양이가 없음
- 앱 ZIP 약 68.7MiB
...
```bash
./Scripts/build-cat-packs.sh K04
unzip -l dist/cat-packs/Meownitor-Cat-K04-v1.zip
```

스크립트는 다음 중 하나라도 틀리면 실패해야 합니다.

- 배포할 15개 동작 이름에 대응하는 raw 디렉터리
- 같은 15개 동작의 intermediate 디렉터리
- 같은 15개 동작의 비어 있지 않은 chroma 보고서
- 같은 15개 동작의 비어 있지 않은 reflection 보고서
- WebP 스트립 15개
- 모든 스트립 `8778×1254`
...
1. 설정에서 Elsa·Hodu·Cheese·Mochi가 설치됨으로 표시되고 다운로드·삭제 버튼이 없는지 확인합니다.
2. 미설치 고양이는 “제작 및 검수 중” 또는 다운로드 크기를 표시하고 미리보기를 비활성화합니다.
3. Release 팩 다운로드 후 자동 선택·미리보기·재실행 지속성을 확인합니다.
4. 삭제 후 Elsa로 복귀하고 Application Support의 해당 ID만 사라지는지 확인합니다. 기본 4종은 삭제할 수 없어야 합니다.
